...
from lost_in_the_middle.metrics import best_subspan_em, ems
...
def main(
    ...
    if q_set_path is not None:
        import pickle
        with open(q_set_path, 'rb') as f:
            q_set = pickle.load(f)
    all_examples = []
    with xopen(input_path) as fin:
        for line in fin:
            input_example = json.loads(line)
            if q_set_path is not None and input_example['question'] not in q_set:
                continue
...
if __name__ == "__main__":
    ...
    parser.add_argument("--q-set-path", help="Question text set", default=None)
