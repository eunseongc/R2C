...
def main(args, logger):
    ...
    prompt_len_list_org = []
    for qas in test_data_org:
        prompt = get_prompt(qas['ctxs'], qas, args.dataset, args.use_org_idx)
    ...
    if 'longbench' in args.dataset:
        ...
        logger.info(f"> As the dataset is longbench, n_contexts is set to None")
    ...
    if 'longbench' in args.dataset: ## args.dataset == 'longbench_samsum' or 'dpr_nq_20' / args.output_root == 'compressed_qa_data/0423'
        output_path = os.path.join(args.output_root, f"{args.dataset}_{output_file_name}.json")
    else:
        output_path = os.path.join(args.output_root, args.dataset, f"{output_file_name}.jsonl.gz")
