...
class T5Pooler(nn.Module):
    ...
    def forward(
        ...
        if sentence_spans is not None:
            ## activation 필요
            pooled_output_passages = self.activation(pooled_output[:, 0])
            ...
            for b, b_spans in enumerate(sentence_spans):
                for qas, qas_spans in enumerate(b_spans):
                    ...
                    else: ## No sentence spans because of text_maxlength, e.g., question only exceeds text_maxlength
...
class CustomT5ForConditionalGeneration(T5ForConditionalGeneration):
    ...
    def forward(
        ...
        auto_regression: Optional[bool] = False, ## ↓ eun's
        ...
        if encoder_outputs is None:
            ...
            encoder_outputs, cross_encoder_loss, sent_loss, probs = self.encoder(
                input_ids=input_ids,
        ...
        if labels is not None:
            ...
            #     print("cross_encoder_loss: ", cross_encoder_loss.item())
            #     print("sent_loss: ", sent_loss.item())
    ...
    def prepare_inputs_for_generation(
        ...
        return {
            ...
            "cross_attn_head_mask": cross_attn_head_mask,
            "use_cache": use_cache,
            "sentence_spans": sentence_spans, ## eun's addition
